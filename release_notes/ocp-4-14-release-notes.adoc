...
* Before this update, the node-exporter could consume large amounts of CPU resources in some configurations. This update limits the number of CPUs that can be used simultaneously by node-exporter. The number of CPUs is now set to `4` or to the number of CPUs available on the host, whichever is smaller. You can override this limit by using the `maxProcs` parameter in the config map settings for node-exporter. See xref:../monitoring/config-map-reference-for-the-cluster-monitoring-operator.html#nodeexporterconfig[NodeExporterConfig] for more information. link:https://issues.redhat.com/browse/OCPBUGS-13153[*OCPBUGS-13153*]
