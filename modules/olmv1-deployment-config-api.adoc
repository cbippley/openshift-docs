...
[role="_abstract"]
Customize operator pod deployments by using the deployment configuration API in the `ClusterExtension` resource.
...
Specify deployment configuration in the `spec.config.inline.deploymentConfig` field as a JSON object.
...
+
--
`deploymentConfig:`:: Deployment configuration object.
