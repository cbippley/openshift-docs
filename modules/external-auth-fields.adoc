...
a|Optional. JWT token claim used for groups mapping. Set either `claim` or `expression`, not both. Length: 1-256 characters.

|`claimMappings.groups.expression`
a|Optional (Technology Preview). CEL expression that produces a string or string array from JWT token claims.

Access claims via `claims` variable (for example, `claims.groups` or `claims.foo.bar` for nested claims).
...
a|Optional. JWT token claim used for username mapping. Set either `claim` or `expression`, not both. Length: 1-256 characters.

|`claimMappings.username.expression`
a|Optional (Technology Preview). CEL expression that produces a string value from JWT token claims. Must result in a single string.

Access claims via `claims` variable (for example, `claims.email` or `claims.foo.bar` for nested claims).
...
a|Optional (Technology Preview). Required when `type` is `CEL`. Contains `expression` (CEL expression to evaluate) and `message` (error text).
...
Access claims via `claims` variable using dot notation (for example, `claims.sub` or `claims.foo.bar`).
...
|`issuer.discoveryURL`
a|Optional (Technology Preview). Custom OIDC discovery endpoint URL. Must be a valid HTTPS URL and differ from `issuer.issuerURL`.

When not specified, {product-title} constructs the discovery URL using the standard OIDC format: `{issuerURL}/.well-known/openid-configuration`.
...
|`userValidationRules`
a|Optional (Technology Preview). Validation rules for user objects created from authenticated tokens. All rules must pass (AND operation).

Each rule contains `expression` (must evaluate to `true`) and `message` (error text).

Access user via `user` variable: `user.username` (string), `user.groups` (array), `user.uid` (string), `user.extra` (map).
