...
[role="_abstract"]
Migrate from local etcd encryption (`aescbc` or `aesgcm`) to external KMS encryption to centralize key management and improve compliance posture.
...
. Save and exit.
+
Migration starts automatically. The kube-apiserver operator restarts and rolls out new revisions. This typically takes several minutes.
