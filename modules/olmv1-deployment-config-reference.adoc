...
= Deployment configuration field reference

[role="_abstract"]
Deployment configuration field reference and {olmv0} to {olmv1} mapping.

:FeatureName: {olmv1} deployment configuration API
...
Merge with precedence:: Merges with bundle values. Deployment configuration takes precedence on conflicts. Applies to: `env`, `envFrom`
...
`env`:: An array of environment variable objects. Merged with existing container environment variables, with deployment configuration values taking precedence. Each object has:
+
...
`annotations`:: A map of key-value pairs for pod annotations. Annotations from the deployment configuration are merged with bundle annotations. When keys conflict, bundle annotations take precedence.
+
.Example annotations
[source,yaml]
----
