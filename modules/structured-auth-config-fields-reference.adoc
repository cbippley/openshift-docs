...
|`issuer.discoveryURL`
a|Optional. Custom OIDC discovery endpoint URL for retrieving identity provider metadata from a non-standard location.
...
a|Optional. Array of validation rules for JWT token claims using Common Expression Language (CEL) expressions. All rules must evaluate to `true` for authentication to succeed (AND operation).
...
CEL expressions access claims via `claims` variable (for example, `claims.sub`).
...
Access claims via `claims` variable: `claims.sub`, `claims.foo.bar` (nested), `has(claims.email)` (existence check).

|`claimValidationRules[].cel.message`
a|Required. Error message displayed when validation fails. Constraints: 1-256 characters.

|`userValidationRules`
a|Optional. Array of validation rules for user objects using CEL expressions. All rules must evaluate to `true` for authentication to succeed (AND operation).

Each rule contains `expression` (must evaluate to `true`) and `message` (error text).

CEL expressions access user object via `user` variable: `user.username` (string), `user.groups` (array), `user.uid` (string), `user.extra` (map).
